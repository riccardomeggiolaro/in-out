...
function createRow(table, columns, item, idInout) {
    ...
    if (!["pid", "weighing-terminal"].includes(itemName)) {
        ...
        const isLatestForBadge = item.is_latest_for_badge ?? (item.access && item.access.is_latest_for_badge);
        const isLatest = (isLatestForVehicle || isLatestForVehicle === null || isLatestForVehicle === undefined) && (isLatestForBadge || isLatestForBadge === null || isLatestForBadge === undefined);
        if (idInout && isReservation && item.is_last && isLatest) actionsCell.appendChild(deleteButton);
        ...
        if (itemName === "access" && item.type === "Prenotazione" && item.mode !== "Transito" && "in_out" in item && !String(item.number_in_out).includes("/")) actionsCell.appendChild(permanentAccess);
                        ...
                        if (firstKey === "weighing") {
                            if (isExpandedRow() && data.data["weighing"].id == currentId && config["use_reservation"] === true) {
                                ...
                                if (liFirstChildP && !liFirstChildP.querySelector('.imgs')) {
                                    const currentLiCount = currentRowExtended.querySelectorAll('li').length;
                                    const newInOutCount = data.data[firstKey].in_out ? data.data[firstKey].in_out.length : 0;
                                    const pairAnimClass = (newInOutCount >= currentLiCount) ? 'soft-deleted' : 'deleted';
                                    liFirstChildP.classList.toggle(pairAnimClass);
