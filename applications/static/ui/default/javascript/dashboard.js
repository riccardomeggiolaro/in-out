...
function processRealtimeObject(obj) {
    if (obj.command_in_executing) {
        ...
        document.getElementById('status').innerText = !hasRealWeight
            ? (data_weight_realtime.status || '--')
            : (data_weight_realtime.status !== undefined ? data_weight_realtime.status : 'N/A');
