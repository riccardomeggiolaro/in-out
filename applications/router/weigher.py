...
def Callback_Data(instance_name: str, instance_node: Union[str, None], data: Data):
	...
	if asyncio.get_event_loop().is_running():
		asyncio.create_task(WEIGHERS[instance_name]["node_sockets"][instance_node].manager_realtime.broadcast(data.dict()))
	else:
		loop = asyncio.get_event_loop()
		loop.run_until_complete(WEIGHERS[instance_name]["node_sockets"][instance_node].manager_realtime.broadcast(data.dict()))
...
async def Weighing(instance: InstanceNameNodeDTO = Depends(get_query_params_name_node), id: Optional[int] = None):
	...
	if id is not None:
		...
		status, data = WEIGHERS[instance.name]["module"].getData(node=instance.node)
	status, status_modope, status_command, error_message = WEIGHERS[instance.name]["module"].setModope(node=instance.node, modope="WEIGHING", data_assigned=data)
	return {
		"instance": instance,
		"command_executed": {
...
@router.delete("/delete/data_in_execution")
...
@router.get("/config_weigher/nodes")
...
@router.get("/config_weigher/node")
...
@router.post("/config_weigher/node")
...
@router.patch("/config_weigher/node")
...
@router.delete("/config_weigher/node")
...
@router.delete("/config_weigher/nodes")
...
@router.get("/config_weigher/connection")
...
@router.patch("/config_weigher/connection")
...
@router.delete("/config_weigher/connection")
...
@router.patch("/config_weigher/time_between_actions/{time}")
