...
from applications.router.weigher.dto import DataDTO, IdentifyDTO
...
class CallbackWeigher(Functions, WebSocket):
	...
	def Callback_Weighing(self, instance_name: str, weigher_name: str, last_pesata: Weight):
		...
		if last_pesata.weight_executed.executed and not "NO" in last_pesata.weight_executed.pid:
			...
			name_file, variables, report = get_data_variables(last_in_out, is_generic=is_test)
				...
				for cam in lb_config.g_config["app_api"]["weighers"][instance_name]["nodes"][weigher_name]["events"]["weighing"]["cams"]:
					if cam["active"]:
						...
						if image_captured_details["image"]:
							...
							i = i + 1
				rele_key = "set_rele_in" if event_type == "in" else "set_rele_out"
				for rele in lb_config.g_config["app_api"]["weighers"][instance_name]["nodes"][weigher_name]["events"]["weighing"].get(rele_key, []):
					modope = "CLOSERELE" if rele["set"] == 0 else "OPENRELE"
	...
	async def WeighingByIdentify(self, request: Request, identify_dto: IdentifyDTO, instance: InstanceNameWeigherDTO = Depends(get_query_params_name_node), token: str = None, notify_identify_code: bool = True):
		...
		elif len(identify_dto.identify) < 5:
			show_message = False
										...
										if current_weigher_data["id_selected"]["id"] != access["id"]:
											...
											if realtime.status == "ST":
												if stable == 3:
													...
													auto_rele_key = "set_rele_out" if weight1 else "set_rele_in"
													for rele in lb_config.g_config["app_api"]["weighers"][instance.instance_name]["nodes"][instance.weigher_name]["events"]["weighing"].get(auto_rele_key, []):
														modope = "CLOSERELE" if rele["set"] == 0 else "OPENRELE"
