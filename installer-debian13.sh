#!/bin/bash
set -e # Termina lo script in caso di errore

SCRIPT_DIR="$(dirname "$0")"
cd "$SCRIPT_DIR"
SOURCE_DIR="$(cd "$(dirname "$(readlink -f "$0")")" && pwd)"
INSTALL_DIR="/opt/in-out"

# Colori per output
GREEN='\033[0;32m'
YELLOW='\033[1;33m'
RED='\033[0;31m'
NC='\033[0m' # No Color

print_warning() {
    echo -e "${YELLOW}[WARN]${NC} $1"
}

print_error() {
    echo -e "${RED}[ERROR]${NC} $1"
}

# Funzione per controllare se un pacchetto è installato
is_installed() {
    dpkg-query -W -f='${Status}' "$1" 2>/dev/null | grep -q "install ok installed"
}

# Funzione per controllare se l'ambiente desktop è MATE
is_mate_desktop() {
    if [ "$XDG_CURRENT_DESKTOP" = "MATE" ] || [ "$DESKTOP_SESSION" = "mate" ]; then
        return 0
    fi

    if pgrep -f "mate-session" >/dev/null 2>&1; then
        return 0
    fi

    if is_installed "mate-desktop" || is_installed "mate-desktop-environment"; then
        return 0
    fi

    return 1
}

# Verifica che lo script venga eseguito su Debian 13
if [ -f /etc/os-release ]; then
    . /etc/os-release
    if [[ "$ID" != "debian" ]]; then
        print_warning "Questo script è ottimizzato per Debian 13. Sistema rilevato: $ID"
        read -p "Vuoi continuare comunque? (s/n) " -n 1 -r
        echo
        if [[ ! $REPLY =~ ^[Ss]$ ]]; then
            exit 1
        fi
    elif [[ "$VERSION_ID" != "13" ]]; then
        print_warning "Questo script è ottimizzato per Debian 13 (Trixie). Versione rilevata: $VERSION_ID ($VERSION_CODENAME)"
        read -p "Vuoi continuare comunque? (s/n) " -n 1 -r
        echo
        if [[ ! $REPLY =~ ^[Ss]$ ]]; then
            exit 1
        fi
    else
        echo "Sistema Debian 13 (Trixie) rilevato"
    fi
else
    print_warning "Impossibile rilevare il sistema operativo"
fi

# Verifica se lo script viene eseguito come root o con sudo
if [ "$EUID" -ne 0 ]; then
    print_error "Questo script deve essere eseguito con sudo o come root"
    exit 1
fi

# Ottieni l'utente che ha invocato sudo (o root se eseguito direttamente come root)
REAL_USER="${SUDO_USER:-$USER}"
echo "Installazione per l'utente: $REAL_USER"

# Installa sudo se non è presente
if ! command -v sudo &>/dev/null; then
    echo "Installazione di sudo..."
    apt update
    apt install -y sudo
    chmod 644 /etc/sudo.conf
    chmod 440 /etc/sudoers
else
    echo "sudo è già installato"
fi

# Aggiungi l'utente corrente al gruppo sudo se non è già presente
if [ "$REAL_USER" != "root" ]; then
    if ! groups "$REAL_USER" | grep -q sudo; then
        echo "Aggiunta dell'utente $REAL_USER al gruppo sudo..."
        usermod -aG sudo "$REAL_USER"
        echo "Utente $REAL_USER aggiunto al gruppo sudo"
    else
        echo "L'utente $REAL_USER è già nel gruppo sudo"
    fi
fi

# Aggiorna l'indice dei pacchetti
echo "Aggiornamento dell'indice dei pacchetti..."
apt update

# Rileva la versione di Python disponibile (Debian 13 usa Python 3.13)
echo "Rilevamento versione Python..."
PYTHON_VERSION=""
for ver in 3.13 3.12 3.11 3.10 3.9; do
    if command -v python$ver &>/dev/null; then
        PYTHON_VERSION=$ver
        echo "Python $ver trovato"
        break
    fi
done

if [ -z "$PYTHON_VERSION" ]; then
    print_warning "Nessuna versione Python 3.9+ trovata, installazione di python3..."
    apt install -y python3
    PYTHON_VERSION=$(python3 --version | awk '{print $2}' | cut -d. -f1,2)
fi

PYTHON_CMD="python$PYTHON_VERSION"
echo "Utilizzo di Python $PYTHON_VERSION"

# Installa python venv se non è presente
VENV_PKG="python$PYTHON_VERSION-venv"
if ! is_installed "$VENV_PKG"; then
    echo "Installazione di $VENV_PKG..."
    apt install -y "$VENV_PKG"
else
    echo "$VENV_PKG è già installato"
fi

# Installa python-dev se non è presente
DEV_PKG="python$PYTHON_VERSION-dev"
if ! is_installed "$DEV_PKG"; then
    echo "Installazione di $DEV_PKG..."
    apt install -y "$DEV_PKG" || {
        print_warning "Impossibile installare $DEV_PKG, provo con python3-dev..."
        apt install -y python3-dev
    }
else
    echo "$DEV_PKG è già installato"
fi

# Installa python3-pip se non è presente
if ! is_installed python3-pip; then
    echo "Installazione di python3-pip..."
    apt install -y python3-pip
else
    echo "python3-pip è già installato"
fi

# Installa python3-tk se non è presente (richiesto per tkinter)
TK_PKG="python$PYTHON_VERSION-tk"
if ! is_installed "$TK_PKG"; then
    echo "Installazione di $TK_PKG (richiesto per tkinter)..."
    apt install -y "$TK_PKG" || {
        print_warning "Impossibile installare $TK_PKG, provo con python3-tk..."
        apt install -y python3-tk
    }
else
    echo "$TK_PKG è già installato"
fi

# Installa CUPS e libcups2-dev (in Debian 13 libcupsimage2 è stato rimosso)
if ! is_installed cups || ! is_installed libcups2-dev; then
    echo "Installazione di CUPS e libcups2-dev..."
    apt install -y cups libcups2-dev
else
    echo "CUPS e le sue dipendenze sono già installati"
fi

# Abilita l'accesso remoto a CUPS (interfaccia web raggiungibile da altri host)
echo "Abilitazione dell'accesso remoto a CUPS..."
cupsctl --remote-admin --remote-any --share-printers
systemctl restart cups
echo "Accesso remoto a CUPS abilitato"

# Installa e abilita Cockpit se non è presente
if ! is_installed cockpit; then
    echo "Cockpit non è installato, installazione in corso..."
    apt install -y cockpit
    echo "Cockpit installato con successo"
else
    echo "Cockpit è già installato"
fi

echo "Abilitazione e avvio del socket Cockpit..."
systemctl enable --now cockpit.socket
echo "Cockpit è attivo"

# Crea la directory di installazione /opt/in-out e copia i file
if [ "$SOURCE_DIR" != "$INSTALL_DIR" ]; then
    echo "Creazione directory di installazione $INSTALL_DIR..."
    mkdir -p "$INSTALL_DIR"
    echo "Copia dei file da $SOURCE_DIR a $INSTALL_DIR..."
    cp -r "$SOURCE_DIR"/* "$INSTALL_DIR/"
    chown -R root:root "$INSTALL_DIR"
    cd "$INSTALL_DIR"
else
    echo "I file sono già in $INSTALL_DIR"
fi

# Crea la directory /var/opt/in-out con le sottocartelle pdf, csv e images
echo "Creazione directory /var/opt/in-out con sottocartelle..."
mkdir -p /var/opt/in-out/{pdf,csv,images}
chown -R root:root /var/opt/in-out
echo "Directory /var/opt/in-out creata con sottocartelle pdf, csv e images"

# Controlla e crea l'ambiente virtuale se non esiste
if [[ ! -d ".venv" ]]; then
    echo "Creazione dell'ambiente virtuale .venv..."
    $PYTHON_CMD -m venv .venv
    VENV_NUOVO=true
else
    VENV_NUOVO=false
    echo "L'ambiente virtuale .venv esiste già"
fi

# Attiva l'ambiente virtuale
source .venv/bin/activate

# Aggiorna pip, setuptools e wheel
echo "Aggiornamento di pip, setuptools e wheel..."
pip install --upgrade pip setuptools wheel

# Installa i requisiti
if [ -f "requirements.txt" ]; then
    echo "Installazione delle dipendenze da requirements.txt..."
    if [ "$VENV_NUOVO" = true ]; then
        pip cache purge
    fi
    pip install -r requirements.txt
else
    print_warning "File requirements.txt non trovato!"
fi

# Controlla se l'ambiente desktop è MATE e installa system-config-printer
if is_mate_desktop; then
    echo "Ambiente desktop MATE rilevato"
    if ! is_installed system-config-printer; then
        echo "Installazione di system-config-printer per MATE..."
        apt install -y system-config-printer
    else
        echo "system-config-printer è già installato"
    fi
else
    echo "Ambiente desktop MATE non rilevato"
fi

# Installa le dipendenze necessarie per WeasyPrint
echo "Controllo e installazione delle dipendenze per WeasyPrint..."
WEASYPRINT_DEPS=(
    "libpango-1.0-0"
    "libpangoft2-1.0-0"
    "libpangocairo-1.0-0"
    "libcairo2"
    "libgdk-pixbuf-2.0-0"
    "libffi-dev"
    "shared-mime-info"
    "libcairo2-dev"
    "libpango1.0-dev"
    "libgdk-pixbuf-2.0-dev"
)

INSTALL_NEEDED=false
for dep in "${WEASYPRINT_DEPS[@]}"; do
    if ! is_installed "$dep"; then
        print_warning "Dipendenza mancante: $dep"
        INSTALL_NEEDED=true
    fi
done

if [ "$INSTALL_NEEDED" = true ]; then
    echo "Installazione delle dipendenze per WeasyPrint..."
    apt install -y "${WEASYPRINT_DEPS[@]}"
else
    echo "Tutte le dipendenze per WeasyPrint sono già installate"
fi

# Installa le altre dipendenze di sviluppo per Python
DEV_LIBS=("libjpeg-dev" "libxml2-dev" "libxslt1-dev")
INSTALL_DEV_NEEDED=false
for lib in "${DEV_LIBS[@]}"; do
    if ! is_installed "$lib"; then
        INSTALL_DEV_NEEDED=true
        break
    fi
done

if [ "$INSTALL_DEV_NEEDED" = true ]; then
    echo "Installazione delle librerie di sviluppo per Python..."
    apt install -y "${DEV_LIBS[@]}"
else
    echo "Le librerie di sviluppo per Python sono già installate"
fi

# Installa ufw se non è presente
if ! command -v ufw &>/dev/null; then
    echo "Installazione di ufw (Uncomplicated Firewall)..."
    apt install -y ufw
else
    echo "ufw è già installato"
fi

# Configura le regole del firewall
echo "Configurazione delle regole del firewall..."
ufw allow 80/tcp comment 'HTTP'
ufw allow 443/tcp comment 'HTTPS'
ufw allow 22/tcp comment 'SSH'
ufw allow 631/tcp comment 'CUPS'
sudo ufw allow 9090/tcp

# Abilita ufw se non è già attivo
if ! ufw status | grep -q "Status: active"; then
    print_warning "Abilitazione di ufw..."
    ufw --force enable
else
    echo "ufw è già attivo"
fi

# Esegui l'installer nella cartella tmt-cups
TMT_CUPS_DIR="$INSTALL_DIR/printers-driver/tmt-cups"
if [ -d "$TMT_CUPS_DIR" ]; then
    echo "Cartella tmt-cups trovata, esecuzione dell'installer..."
    if [ -f "$TMT_CUPS_DIR/install.sh" ]; then
        cd "$TMT_CUPS_DIR"
        bash install.sh
        cd "$INSTALL_DIR"
        echo "Installer tmt-cups completato"
    else
        print_warning "File install.sh non trovato in $TMT_CUPS_DIR"
    fi
else
    print_warning "Cartella tmt-cups non trovata in $INSTALL_DIR"
fi

# Esegui l'installer nella cartella VKP80_CUPSDrv-PKG
VKP80_CUPSDrv_DIR="$INSTALL_DIR/printers-driver/VKP80_CUPSDrv-PKG"
if [ -d "$VKP80_CUPSDrv_DIR" ]; then
    echo "Cartella VKP80_CUPSDrv-PKG trovata, esecuzione dell'installer..."
    if [ -f "$VKP80_CUPSDrv_DIR/VKP80_CUPSDrv.sh" ]; then
        cd "$VKP80_CUPSDrv_DIR"
        bash VKP80_CUPSDrv.sh
        cd "$INSTALL_DIR"
        echo "Installer VKP80_CUPSDrv-PKG completato"
    else
        print_warning "File VKP80_CUPSDrv.sh non trovato in $VKP80_CUPSDrv_DIR"
    fi
else
    print_warning "Cartella VKP80_CUPSDrv-PKG non trovata in $INSTALL_DIR"
fi

# Esegui l'installer nella cartella VKP80III_CUPSDrv-PKG
VKP80III_CUPSDrv_DIR="$INSTALL_DIR/printers-driver/VKP80III_CUPSDrv-PKG"
if [ -d "$VKP80III_CUPSDrv_DIR" ]; then
    echo "Cartella VKP80III_CUPSDrv-PKG trovata, esecuzione dell'installer..."
    if [ -f "$VKP80III_CUPSDrv_DIR/VKP80III_CUPSDrv.sh" ]; then
        cd "$VKP80III_CUPSDrv_DIR"
        bash VKP80III_CUPSDrv.sh
        cd "$INSTALL_DIR"
        echo "Installer VKP80III_CUPSDrv-PKG completato"
    else
        print_warning "File VKP80III_CUPSDrv.sh non trovato in $VKP80III_CUPSDrv_DIR"
    fi
else
    print_warning "Cartella VKP80III_CUPSDrv-PKG non trovata in $INSTALL_DIR"
fi

# Copia la cartella "Driver e stampe" in /home/baronpesi/Scaricati
DRIVER_SRC="$SOURCE_DIR/Driver e stampe"
if [ -d "$DRIVER_SRC" ]; then
    echo "Copia della cartella 'Driver e stampe' in /home/baronpesi/Scaricati..."
    mkdir -p "/home/baronpesi/Scaricati"
    cp -r "$DRIVER_SRC" "/home/baronpesi/Scaricati/"
    chown -R baronpesi:baronpesi "/home/baronpesi/Scaricati/Driver e stampe"
    echo "Cartella 'Driver e stampe' copiata con successo"
else
    print_warning "Cartella 'Driver e stampe' non trovata in $SOURCE_DIR"
fi

# Crea il servizio systemd se non esiste
SERVICE_FILE="/etc/systemd/system/in-out.service"
if [ ! -f "$SERVICE_FILE" ]; then
    START_SCRIPT="$INSTALL_DIR/start.sh"

    if [ ! -f "$START_SCRIPT" ]; then
        print_error "File start.sh non trovato in $INSTALL_DIR"
        exit 1
    fi

    chmod +x "$START_SCRIPT"

    echo "Creazione del servizio systemd in-out.service..."
    cat << EOF > "$SERVICE_FILE"
[Unit]
Description=In-Out Service
After=network.target

[Service]
ExecStart=$START_SCRIPT
WorkingDirectory=$INSTALL_DIR
User=root
Group=root
Restart=always
RestartSec=10

[Install]
WantedBy=multi-user.target
EOF
    echo "Servizio systemd in-out.service creato"
else
    echo "Il servizio systemd in-out.service esiste già"
fi

# Ricarica systemd per prendere in considerazione il nuovo servizio
echo "Ricaricamento della configurazione systemd..."
systemctl daemon-reload

# Abilita il servizio per farlo partire all'avvio
echo "Abilitazione del servizio in-out..."
systemctl enable in-out.service

# Avvia il servizio
echo "Avvio del servizio in-out..."
systemctl start in-out.service

# Verifica lo stato del servizio
sleep 2
if systemctl is-active --quiet in-out.service; then
    echo "Il servizio in-out è stato avviato con successo"
else
    print_error "Il servizio in-out non si è avviato correttamente"
    echo "Controllare lo stato con: sudo systemctl status in-out.service"
    exit 1
fi

echo "========================================"
echo "Installazione completata con successo!"
echo "========================================"
echo "Il servizio in-out è attivo e verrà eseguito all'avvio del sistema"
echo ""
echo "Comandi utili:"
echo "  - Stato servizio: sudo systemctl status in-out.service"
echo "  - Stop servizio:  sudo systemctl stop in-out.service"
echo "  - Restart:        sudo systemctl restart in-out.service"
echo "  - Log servizio:   sudo journalctl -u in-out.service -f"