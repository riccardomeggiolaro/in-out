...
if [ -f /etc/os-release ]; then
    ...
    if [[ "$ID" != "debian" ]]; then
        ...
        print_info "Sistema Debian 13 (Trixie) rilevato"
    fi
else
...
if ! is_installed cups || ! is_installed libcups2-dev; then
    print_info "Installazione di CUPS e libcups2-dev..."
...
print_info "Abilitazione dell'accesso remoto a CUPS..."
...
print_info "Creazione directory /var/opt/in-out con sottocartelle..."
mkdir -p /var/opt/in-out/{pdf,csv,images}
chown -R root:root /var/opt/in-out
print_info "Directory /var/opt/in-out creata con sottocartelle pdf, csv e images"
...
if [ -d "$VKP80_CUPSDrv_DIR" ]; then
    print_info "Cartella VKP80_CUPSDrv-PKG trovata, esecuzione dell'installer..."
    if [ -f "$VKP80_CUPSDrv_DIR/VKP80_CUPSDrv.sh" ]; then
        cd "$VKP80_CUPSDrv_DIR"
        bash VKP80_CUPSDrv.sh
        cd "$INSTALL_DIR"
        print_info "Installer VKP80_CUPSDrv-PKG completato"
    else
...
if [ -d "$VKP80III_CUPSDrv_DIR" ]; then
    print_info "Cartella VKP80III_CUPSDrv-PKG trovata, esecuzione dell'installer..."
    if [ -f "$VKP80III_CUPSDrv_DIR/VKP80III_CUPSDrv.sh" ]; then
        cd "$VKP80III_CUPSDrv_DIR"
        bash VKP80III_CUPSDrv.sh
        cd "$INSTALL_DIR"
        print_info "Installer VKP80III_CUPSDrv-PKG completato"
    else
...
print_info "  - Restart:        sudo systemctl restart in-out.service"
